...
class PessoaController {
  static async pegaTodasAsPessoasAtivas(req, res) {
    try {
      const todasAsPessoasAtivas = await pessoaService.pegaRegistrosAtivos();
  ...
  static async cancelaPessoa(req, res) {
    const { estudanteId } = req.params;
    try {
      await pessoaService.cancelaPessoaEMatriculas(Number(estudanteId));
      return res.status(200).json({
        message: `Matrículas ref. ao estudante ${estudanteId} foram canceladas`,
      });
    } catch (error) {
      return res.status(500).json(error.message);
    }
